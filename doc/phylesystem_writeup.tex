...
The total size of the stored data (in JSON format with a line ending after 
    every field) is only around 150MB.  [does this include version history????]
...
We opted to version JSON files 
    due to its wide uptake in the software development community, and in particular 
...
For new files deposited through the web application, the interface strongly encourages application of a CC0 copyright waiver, 
    ...
    [???? consider citing http://dx.doi.org/10.1186/1756-0500-7-79 -
      shameless JAR self-promotion]
...
\begin{enumerate}
    ...
    \item \label{userEditStep} The curator may correct tip taxon assignments with the help of taxonomic name resolution services (the ``taxomachine'' web services, which are described
        [???? this makes it sound ike there are only two kinds of edit.  in face there are many.]
        elsewhere) or fix the rooting information about the tree. The study data is modified in the memory space of the 
    ...
    \item \label{putStep} To save, the curator application uses an
      HTTP request to the API and includes the data (in \nexson), the starting SHA, and a commit message to support sensible history. 
        ...
        \jfacomment{It does seem to raise the topic of RESTful style. Not sure this is this relevant here. [??? it isn't]}
...
[???? capitalize subsection headings, and no hyphens]
...
[???? an example/figure might be nice here, if space permits]
