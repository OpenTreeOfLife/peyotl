...
Hosting this datastore on GitHub provides a open access to the data store using tools
    familiar to many developers and researchers.
...
The state of phyloinformatic archiving described above suggests the need for a datastore that allows the 
    community of biologists to dynamically improve the accuracy and re-usability of published phylogenetic statements.
...
The \otol project has placed all of the code for the \ps under permissive, BSD-style licenses and the
...
The \nexson 1.0 syntax augments the badgerfish convention by using the \^{} character at the front of a
    property name to indicate that the key-value property that corresponds to a ``meta'' element of NeXML.
This makes the files smaller and makes access of a property a faster lookup operation (rather than a search
    through a meta list).\ejmcomment{I changed this sentence, but may have changed the meaning, is this right?}

The \nexson 1.2 syntax reorganizes some of the elements of a NeXML.
In this version of the syntax, the OTUs, trees, nodes, and edges are contained in objects with the IDs of the entities \ejmcomment{not clear on what entities means here} as keys.
...
If a study has been added, deleted or edited by a user and the update was merged to the master branch of the server data store,
    ...
    of the repository.
This assures that the publicly visible version of the data is updated very quickly, and it also allows us to use
    GitHub webhooks to trigger other operations.
...
Ideally, such a system would version a file formats in which:
    ...
    the order of elements in the serialize file can be made consistent.
