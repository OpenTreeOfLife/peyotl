...
if test -z $GITHUB_OAUTH_TOKEN
then
    echo 'GITHUB_OAUTH_TOKEN must be in your environment if you want to run the web-service tests that write'
    exit 0
fi
num_checks=0
num_fails=0
echo_and_demand_succeeds python tests/ws_tests/write_tests/test_collections_api.py
stf=0
if test ${num_fails} -gt 0
then
    num_passed=$(expr $num_checks - $num_fails)
    echo "passed ${num_passed} out of ${num_checks} writing web service tests"
else
    echo "passed ${num_checks} out of ${num_checks} writing web service tests"
fi
exit ${num_fails}