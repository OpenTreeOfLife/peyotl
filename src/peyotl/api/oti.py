#!/usr/bin/env python
from peyotl.utility import get_logger, get_config_object
...
class _OTIWrapper(_WSWrapper):
    ...
    def find_all_studies(self, include_trees=False, verbose=False):
        ...
        data = {'includeTreeMetadata': include_trees,
                'verbose': verbose, }
        response = self.json_http_post(url, data=json.dumps(data))
        return response
    ...
    def trigger_index(self, phylesystem_api, study_id):
        ...
        return self.json_http_post(url, data=json.dumps(data))
