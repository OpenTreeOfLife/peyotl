#!/usr/bin/env python
from peyutil.dict_wrapper import FrozenDictAttrWrapper, FrozenDictWrapper
...
class _TaxomachineAPIWrapper(_WSWrapper):
    ...
    def TNRS(self,
        ...
        resp = self.json_http_post(uri, data=json.dumps(data))
    ...
    def autocomplete(self, name, context_name=None, include_dubious=False):
        ...
        if self.use_v1:
            ...
            if include_dubious:
                data['include_dubious'] = True
        return self.json_http_post(uri, data=json.dumps(data))
    ...
    def taxon(self, ott_id, include_lineage=False, list_terminal_descendants=False, wrap_response=None):
        ...
        r = self.json_http_post(uri, data=json.dumps(data))
        if 'error' in r:
