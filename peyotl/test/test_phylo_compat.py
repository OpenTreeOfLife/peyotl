...
class TestCompat(unittest.TestCase):
    ...
    def testCompareSetsAsSplits(self):
        ...
        self.assertEqual(compare_sets_as_splits(set([1, 2, 4]), set([4, 92]), uni),
                         SplitComparison.UNROOTED_INCOMPATIBLE)
        self.assertEqual(compare_sets_as_splits(set([1, 2, 4]), set([1, 2, 3]), uni),
                         SplitComparison.UNROOTED_INCOMPATIBLE)
        self.assertEqual(compare_sets_as_splits(set([1, 2, 4]), set([1, 2, 4]), uni),
                         SplitComparison.ROOTED_EQUIVALENT)
        self.assertEqual(compare_sets_as_splits(set([1, 2, 4]), set([3, 92]), uni),
                         SplitComparison.UNROOTED_EQUIVALENT)
        self.assertEqual(compare_sets_as_splits(set([1, 2, 4]), set([4, 3, 92]), uni),
                         SplitComparison.UNROOTED_COMPAT)
        self.assertEqual(compare_sets_as_splits(set([1, 2, 4]), set([2, 1]), uni),
