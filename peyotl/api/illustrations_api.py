#!/usr/bin/env python
from peyotl.illustrations.illustrations_umbrella import IllustrationStore, IllustrationStoreProxy
...
_GET_FROM_VALUES = ('local',  # only from local copy of illustrations
                    'external',  # *DEFAULT* from external URLs
                    'api',)  # from the GET calls of the illustrations-api
...
class _IllustrationsAPIWrapper(_WSWrapper):
    ...
    def docstore_obj(self):
        if self._docstore_obj is None:
            if self._src_code == _GET_LOCAL:
                self._docstore_obj = IllustrationStore(repos_dict=self._locals_repo_dict)
            else:
                self._docstore_obj = IllustrationStoreProxy(self.store_config)
    ...
    def get_external_url(self, illustration_id):
        if self._src_code == _GET_API:
            return self._remote_external_url(illustration_id)['url']
        return self.docstore_obj.get_external_url(illustration_id)

    @property
    def illustration_list(self):
        if self._src_code == _GET_API:
            return self._remote_illustration_list()
        return self.docstore_obj.get_illustration_ids()
    ...
    def get(self, illustration_id, content=None, **kwargs):
        ...
        see:
        """
        assert ILLUSTRATION_ID_PATTERN.match(illustration_id)
        r = self.get_illustration(illustration_id)
    ...
    def get_illustration(self, illustration_id):
        if self._src_code == _GET_EXTERNAL:
            url = self.get_external_url(illustration_id)
            json = self.json_http_get(url)
            r = {'data': json}
        elif self._src_code == _GET_LOCAL:
            json, sha = self.docstore_obj.return_doc(illustration_id)  # pylint: disable=W0632
            r = {'data': json,
                 'sha': sha}
        else:
            assert self._src_code == _GET_API
            r = self._remote_get_illustration(illustration_id)
        return r
...
''')
    ...
    def _remote_illustration_list(self):
        """Returns a list of strings which are the illustration IDs"""
        uri = '{}/illustrations/illustration_list'.format(self._prefix)
        return self.json_http_get(uri)

    def _remote_push_failure(self):
        """Returns a list of strings which are the illustration IDs"""
        uri = '{}/illustrations/push_failure'.format(self._prefix)
        return self.json_http_get(uri)

    def unmerged_branches(self):
        uri = '{}/illustrations/unmerged_branches'.format(self._prefix)
        return self.json_http_get(uri)

    def post_illustration(self,
                       json,
                       commit_msg=None):
        assert json is not None
        uri = '{d}/illustration'.format(d=self._prefix)
    ...
    def _remote_store_config(self):
        uri = '{d}/illustrations/store_config'.format(d=self._prefix)
        return self.json_http_get(uri)

    def _remote_external_url(self, illustration_id):
        uri = '{d}/illustrations/external_url/{i}'.format(d=self._prefix, i=illustration_id)
        return self.json_http_get(uri)

    def _remote_get_illustration(self, illustration_id):
        uri = '{d}/illustration/{i}'.format(d=self._prefix, i=illustration_id)
        return self.json_http_get(uri)  # , params=None, text=False)


# noinspection PyPep8Naming
def IllustrationsAPI(domains=None, **kwargs):
    return APIWrapper(domains=domains, **kwargs).wrap_illustrations_api(**kwargs)
