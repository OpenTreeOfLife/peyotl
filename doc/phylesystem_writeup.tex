...
\newcommand{\jfacomment}[1]{{\color{red} \textsc{#1}}\xspace}
...
The xml-based NexML specification \citep{NeXML} defines such a format, but it is not widely used by tree estimation tools.
    ...
    \jfacomment{This section nicely captures the tension between data quality
    and ease of submission/curation. I kind of want to say ``We hope to
    overcome this by providing better tools.''}
...
We anticipate that most of the trees stored in \ps will be associated with permanent, static archives
    ...
    \jfacomment{To my eye, this is pretty vague on the ultimate relationship of
    phylesystem data to Dryad/TreeBASE archives. Are curated changes and
    refinements shared with them? Or are their data used as inputs to
    phylesystem, which now has the ``real'' (best, richest) data? In any case,
    will these repositories link BACK to our phylesystem?}
...
The data model used for the JSON is a close derivative of the NeXML standard for data interoperability \citep{NeXML}.
    \jfacomment{We might want to mention up-front that we're providing lossless conversion from more common formats! Otherwise this looks like \url{http://xkcd.com/927/}. }
...
We hope that adopting a truly distributed backend for the datastore will reinforce the goals
    ...
    \jfacomment{In so doing, we open ourselves to the possibility of the database forking based on some future dispute or other shenanigans, resulting in fragmented efforts. Any point in mentioning this?}
...
Thus, there is no private database that would be required for someone to fork and maintain their
    ...
    \jfacomment{Are we expecting (or even prepared) for a private fork to push its data back to the main corpus? It's not clear in the paper.}
